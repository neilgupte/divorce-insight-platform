...
const LabourDashboard = ({ onNewAnalysis }: LabourDashboardProps) => {
  ...
  // Mock market location - can be dynamic or from context
  const marketInfo = {
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-2">
        <div>
          ...
          <MarketDisplay {...marketInfo} />
        </div>
        <Button onClick={handleNewAnalysis} className="gap-2 self-start sm:self-auto">
              ...
              {mockAnalyses.map((analysis) => (
                <TableRow key={analysis.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        analysis.supplyRisk === "green"
                          ? "outline"
                          : analysis.supplyRisk === "amber"
                          ? "secondary"
                          : "destructive"
                      }
                    >
