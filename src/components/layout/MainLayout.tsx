...
const MainLayout = () => {
  ...
  const labourPlanningMenuItems = [
    ...
    { name: "Create Labour Model", icon: Clock, path: "/labour-planning/create", permission: "" },
    { name: "Task Mapping", icon: GitMerge, path: "/labour-planning/task-mapping", permission: "" },
    { name: "Model Runs", icon: History, path: "/labour-planning/model-runs", permission: "" },
    { name: "Locations", icon: MapPin, path: "/labour-planning/locations", permission: "" },
    { name: "Settings", icon: Settings, path: "/labour-planning/settings", permission: "" },
  ];

  const labourPotentialMenuItems = [
    { name: "Dashboard", icon: LayoutDashboard, path: "/labour-potential/dashboard", permission: "" },
    { name: "Search Location", icon: Map, path: "/labour-potential/search", permission: "" },
    { name: "Supply vs Demand", icon: FileText, path: "/labour-potential/supply-vs-demand", permission: "" },
    { name: "Market Reports", icon: FileBox, path: "/labour-potential/reports", permission: "" },
    { name: "Settings", icon: Settings, path: "/labour-potential/settings", permission: "" },
  ];
  ...
  const realEstateMenuItems = [
    ...
    { name: "Dashboard 2", icon: LayoutDashboard, path: "/dashboard2", permission: "logs:view" },
  ];

  const adminMenuItems = [
    { name: "User Management", icon: Users, path: "/users", permission: "users:manage" },
    { name: "Settings", icon: Settings, path: "/settings", permission: "settings:manage" }
  ];
      ...
      >
        <div className="flex h-full flex-col">
            ...
            {!sidebarCollapsed ? (
              ...
              <div className="w-full px-2">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start" className="w-[300px] p-3" sideOffset={8}>
                    {/* Empty collapsed fallback */}
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
            )}
          ...
          <nav className="flex-1 space-y-1 px-2 py-2">
            ...
            {filteredAdminMenuItems.length > 0 && activeModule === "realestate" && (
              <>
